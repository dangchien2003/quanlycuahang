...
if (checkRequest($_GET, ["malk"])) {
    $sql = "SELECT * FROM linhkien where linhkien.trangthai in (1, 6) and linhkien.malinhkien = ?";
...
if ($error['e']) {
    header("Location: ./cuahanglinhkien.php?status=400&message=" . $error['message']);
    exit();
}
?>
<div class="row">
    ...
    <div class="col-lg-9 sdp tp" style="min-height: 1000px;">
        <div class="box ">
            <form action="../handle/banhang.php?mh=lk" method="post" class="thongtinsanpham">
                <input type="password" name="malk" class="d-none" value="<?php echo $row['malinhkien']?>">
